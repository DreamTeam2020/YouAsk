...
def getQuestion(id):
    try:
        connection, cursor = dbConnect()
        cursor.execute("SELECT * FROM ask_questions WHERE id=3")#  %d") % int(id)    # Edit this %d not working
        fetch=cursor.fetchall()
        dbClose(connection, cursor)
...
def getAnswers(question_id):
    try:
        connection, cursor=dbConnect()
        cursor.execute("SELECT * FROM ask_answers WHERE question_id=3") #%d") % int(question_id)    # Edit this %d not working
        if cursor.rowcount>0:
